...
//   - Variable column heights (NEW): each map cell encodes both color & height
//
// Map data format (in map_doutb):
//   bits [3:0]  : color  (palette index 0-15)
//   bits [6:4]  : height (0 = air, 1-7 = column height in world units)
//   bits [31:7] : unused
//
// A cell with height == 0 is treated as air, regardless of the color bits.
// Boundary walls (when ray exits the 32x32 map) are forced to maximum height (7).
//
// Eye height is fixed at z = 1.0 world units. The player_z input is unused.
// =============================================================================
...
module raycaster_engine (
    ...
    input  logic signed [31:0] player_x, player_y, player_z,  // player_z is UNUSED
    ...
    output logic [17:0] vram_w_addr,  // EXPANDED TO 18 BITS FOR DOUBLE BUFFERING
    output logic [3:0]  vram_w_data,
    output logic        vram_we,
    output logic        write_buffer  // NEW: EXPORT BUFFER STATE TO TOP LEVEL
);
    ...
    enum logic [4:0] {
        ...
        CALC_HEIGHT_MUL, CALC_HEIGHT_ADD, DRAW_SEG
    } state;
    ...
    logic hit, side;
    logic [3:0] hit_color;
    logic [2:0] hit_height;            // NEW: 1-7 valid (0 = no wall / depth-exceeded)
    ...
    logic [8:0] clip_y;            // farther blocks may only draw above this y
    ...
    always_ff @(posedge clk) begin
        if (reset) begin
            ...
            write_buffer       <= 1'b0; // NEW: Initialize double buffer
        end else begin
            vram_we <= 0;
            ...
            case (state)
                ...
                INIT: begin
                    if (screen_x < 320) begin
                        ...
                        screen_x     <= 9'd0;
                        write_buffer <= ~write_buffer; // NEW: Toggle buffer for next frame
                    end
                end
                
                DRAW_BG: begin
                    // NEW: Write to upper half of VRAM if write_buffer is 1
                    if (write_buffer)
                    ...
                    if (screen_y < 8'd120)
                        vram_w_data <= 4'h0; // CHANGED: 4'h0 is the Light Blue Sky
                    else
                ...
                // ============================================================
                // DDA_CHECK (MODIFIED)
                // Hit detection now uses bits [6:4] (height) instead of [3:0].
                // Boundary walls are forced to maximum height (7).
                // Cells with height == 0 are treated as air, regardless of
                // the color bits.
                // ============================================================
                DDA_CHECK: begin
                    if (nextMapX <= 0 || nextMapX >= 31 || nextMapY <= 0 || nextMapY >= 31) begin
                        ...
                        hit_color           <= 4'h1;
                        hit_height          <= 3'd7;
                        ray_stop_after_hit  <= 1'b1;   // boundary wall ends this ray
                        state               <= CALC_DIST;
                    end else if (map_doutb[6:4] != 3'd0) begin
                        hit                 <= 1'b1;
                        hit_color           <= map_doutb[3:0];
                        hit_height          <= map_doutb[6:4];
                        ray_stop_after_hit  <= 1'b0;   // normal block: keep looking behind it
                        state               <= CALC_DIST;
                    end else begin
                        state <= DDA_STEP;
                    end
                ...
                // ============================================================
                // CALC_HEIGHT_ADD (MODIFIED)
                // Eye height is fixed at z = 1.0 world units (player_z unused).
                //
                // Projection of a world Z coordinate to screen Y:
                //   screen_y(world_z) = 120 - (world_z - eye_z) * sLH
                //
                // Wall column extends from world_z = 0 up to world_z = hit_height.
                //   drawEnd   = screen_y(0)             = 120 + sLH
                //   drawStart = screen_y(hit_height)   = drawEnd - hit_height*sLH
                //
                // hit_height is 3 bits (1..7), so wall_pixels is computed inline
                // with a small (32 x 3) multiply.
                // ============================================================
                CALC_HEIGHT_ADD: begin
                    ...
                    // This hit has a visible part above previous closer blocks
                        ...
                        // Everything below this top edge is now blocked for farther blocks
                        clip_y <= c_ds;
                
                        state <= DRAW_SEG;
                    end
                
                    // This block is fully hidden by a closer block
                    else begin
                        if (ray_stop_after_hit) begin
                            screen_x <= screen_x + 1'b1;
                            state    <= INIT;
                ...
                // ============================================================
                // DRAW_COL (UNCHANGED)
                // Sky / wall / floor selection driven by drawStart / drawEnd.
                //
                //   Step 1 - Side shading (Y-face walls one shade darker)
                //   Step 2 - Distance fog (one or two shade_dark passes)
                // ============================================================
                DRAW_SEG: begin
                    ...
                    if (screen_y == drawSegEnd) begin
                        if (ray_stop_after_hit || clip_y == 9'd0) begin
                            ...
                            state <= DDA_STEP;   // continue ray behind this block
                        end
                    end else begin
                        screen_y <= screen_y + 1'b1;
                    end
