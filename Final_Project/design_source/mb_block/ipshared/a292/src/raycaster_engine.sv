            ...
            case (state)
                ...
                DDA_STEP: begin
                    ...
                    if (depth_count > 64) begin
                        hit        <= 1;
                        hit_color  <= 4'h0;
                        hit_height <= 3'd0;     // NEW: no wall drawn when depth-exceeded
                        state      <= CALC_DIST;
                    end else begin
                        tempNextMapX = mapX;
                        tempNextMapY = mapY;
                ...
                DDA_CHECK: begin
                    if (nextMapX <= 0 || nextMapX >= 31 || nextMapY <= 0 || nextMapY >= 31) begin
                        hit        <= 1;
                        hit_color  <= 4'h1;
                        hit_height <= 3'd7;            // boundary wall = max height
                        state      <= CALC_DIST;
                    end else if (map_doutb[6:4] != 3'd0) begin
                        hit        <= 1;
                        hit_color  <= map_doutb[3:0];
                        hit_height <= map_doutb[6:4]; // 1..7 from map data
                        state      <= CALC_DIST;
                    end else begin
                        // height == 0 -> air (color bits ignored)
                        state <= DDA_STEP;
                    end
                end
                ...
                CALC_HEIGHT_ADD: begin
                    ...
                    // Variable column height: wall spans hit_height units in world Z.
                    wall_pixels = saved_line_height * $signed({29'd0, hit_height});
 
                    de = 32'sd120 + (saved_line_height >>> 1);   // bottom of wall (world_z = 0)
                    ds = de - wall_pixels;               // top of wall (world_z = hit_height)
                ...
                DRAW_COL: begin
                    ...
                    if      (screen_y < drawStart) vram_w_data <= 4'hB; // sky  (lapis dark)
                    else if (screen_y > drawEnd)   vram_w_data <= 4'h5; // floor (dark green)
                    else                           vram_w_data <= wall_color;
