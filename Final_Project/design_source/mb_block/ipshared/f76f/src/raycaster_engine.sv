...
module raycaster_engine (
    ...
    output logic [17:0] vram_w_addr,  // 18 bits for Double Buffering (153,600 depth)
    output logic [3:0]  vram_w_data,
    output logic        vram_we,
    output logic        write_buffer  // Tells VGA which buffer to read
);
    ...
    // NOW STORES 24 INTEGER BITS AND 8 FRACTION BITS
    ...
    function automatic [3:0] shade_dark(input logic [3:0] c);
        case (c)
            4'h0: shade_dark = 4'h8;
            4'h1: shade_dark = 4'h3;
            4'h2: shade_dark = 4'h5;
            4'h3: shade_dark = 4'hD;
            4'h4: shade_dark = 4'h3;
            4'h5: shade_dark = 4'h5;
            4'h6: shade_dark = 4'hD;
            4'h7: shade_dark = 4'h8;
            4'h8: shade_dark = 4'hB;
            4'h9: shade_dark = 4'h3;
            4'hA: shade_dark = 4'h9;
            4'hB: shade_dark = 4'hB;
            4'hC: shade_dark = 4'hC;
            4'hD: shade_dark = 4'hD;
            4'hE: shade_dark = 4'h1;
            4'hF: shade_dark = 4'h6;
        ...
        if (reset) begin
            ...
            case (state)
               ...
               DIV_HEIGHT_INIT: begin
                    // Shift the base 240 by 16 (15728640), and then by 8 MORE bits
                    // 15728640 << 8 = 4026531840 (0xF0000000). Fits perfectly in 64 bits.
                    div_num      <= 64'd4026531840; 
                ...
                WAIT_HEIGHT: begin
                    div_valid_in <= 1'b0;
                    if (div_valid_out) begin
                        // Grab the standard integer quotient. 
                        // Because we shifted the numerator, this is now a perfect 24.8 Fixed Point decimal!
                ...
                CALC_HEIGHT_ADD: begin
                    ...
                    automatic logic signed [31:0] unit_height;
                    automatic logic signed [31:0] total_height_fp;
                    ...
                    // 1. Shift out the 8 decimals (+128 for perfect rounding) to get the floor line
                    unit_height = (saved_line_height + 32'd128) >>> 8;
                
                    // 2. Multiply height USING the 24.8 decimals to prevent amplification
                    total_height_fp = saved_line_height * $signed({29'd0, hit_height});
                    
                    // 3. Shift out the decimals to get the final wall size
                    wall_pixels = (total_height_fp + 32'd128) >>> 8;
                
                    de = 32'sd120 + unit_height;
                    ds = de - wall_pixels;
