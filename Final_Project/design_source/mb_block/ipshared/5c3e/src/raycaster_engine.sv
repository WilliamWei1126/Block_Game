...
module raycaster_engine (
    ...
    output logic [17:0] vram_w_addr,  // Expanded to 18 bits for 153,600 depth
    output logic [3:0]  vram_w_data,
    output logic        vram_we,
    output logic        write_buffer  // EXPORTED: Tells VGA which buffer to read
);
    ...
    always_ff @(posedge clk) begin
        if (reset) begin
            ...
            write_buffer       <= 1'b0; // Initialize buffer tracker
        end else begin
            vram_we <= 0;
            ...
            case (state)
                INIT: begin
                    if (screen_x < 320) begin
                        ...
                        screen_x     <= 9'd0;
                        // Frame finished! Toggle the buffer for the next frame
                        write_buffer <= ~write_buffer; 
                    end
                end
                
                DRAW_BG: begin
                    // Write to Offset 76,800 if buffer is 1
                    if (write_buffer)
...
automatic logic signed [31:0] safe_dist = (perpWallDist <= 0) ? 32'd1 : perpWallDist;
                    ...
                    // Add half the denominator (safe_dist >> 1) to the numerator to force rounding
                    div_num      <= {32'd0, 32'd15728640} + {32'd0, (safe_dist >> 1)};
                    div_den      <= safe_dist;
